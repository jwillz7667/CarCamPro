...
const pickFloat = (min, max, decimals = 2) => {
...
const LOCALES = ['en-US', 'en-GB', 'es-MX', 'fr-CA', 'de-DE', 'pt-BR', 'ja-JP'];
const TIMEZONES = [
...
const OS_VERSIONS = ['26.0', '26.1', '26.2'];
const APP_VERSIONS = ['1.0.0', '1.0.1', '1.1.0'];
const PRODUCT_IDS = {
    FREE: null,
...
const wipeSeedData = async () => {
    ...
    // get SetNull).
    const userIds = Array.from({ length: PLANS.length }, (_, i) => deterministicUlid(`seed-user-${i.toString().padStart(2, '0')}`));
...
const main = async () => {
    ...
    for (let i = 0; i < PLANS.length; i += 1) {
        ...
        console.log(`• user ${i.toString().padStart(2, '0')} (${plan.tier}, ${plan.subscriptionStatus ?? '—'}): ${first} ${last}`);
...
const seedSubscription = async (params) => {
    ...
    if (plan.tier === 'FREE' || plan.subscriptionStatus === null)
        return null;
...
const seedClips = async (params) => {
    ...
    if (plan.clips === 0 || deviceIds.length === 0)
        return [];
    const ids = [];
    for (let c = 0; c < plan.clips; c += 1) {
        ...
        const status = rng() < 0.92 ? 'UPLOADED' : (rng() < 0.5 ? 'PENDING' : 'FAILED');
        const sizeBytes = BigInt(Math.round(duration *
...
const seedIncidentReports = async (params) => {
    ...
    if (plan.tier !== 'PREMIUM' || plan.incidentReports === 0)
        return;
...
const seedHazardReports = async (params) => {
    ...
    for (let h = 0; h < plan.hazardReports; h += 1) {
        ...
        const salt = saltsByDay.get(dayKey) ?? saltsByDay.get(new Date().toISOString().slice(0, 10));
...
const encodeGeohash = (lat, lng, precision = 9) => {
...
const seedHazardVotes = async (users) => {
    ...
    for (const s of allSightings) {
        for (const u of users) {
            if (u.plan.softDeleted)
                continue;
            if (rng() < 0.30) {
                const direction = rng() < 0.85 ? 1 : -1;
                try {
...
const summarize = async () => {
    ...
    for (const t of tables) {
        const rows = await prisma.$queryRawUnsafe(`SELECT COUNT(*)::bigint AS n FROM ${t}`);
